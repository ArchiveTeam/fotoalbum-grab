...
LUA_SHA1 = get_hash(os.path.join(CWD, 'fotoalbum.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--timeout', '30',
            '--tries', 'inf',
            '--domains', 'fotoalbum.ee',
            '--span-hosts',
            '--waitretry', '30',
            '--warc-file', ItemInterpolation('%(item_dir)s/%(warc_file_base)s'),
            '--warc-header', 'operator: Archive Team',
            '--warc-header', 'fotoalbum-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('fotoalbum-item: %(item_name)s'),
...
project = Project(
    title = 'Fotoalbum',
    project_html = '''
    <img class="project-logo" alt="logo" src="https://archiveteam.org/images/6/6d/Fotoalbum-logo.png" height="50px"/>
    <h2>fotoalbum.ee <span class="links"><a href="http://fotoalbum.ee/">Website</a> &middot; <a href="http://tracker.archiveteam.org/fotoalbum/">Leaderboard</a></span></h2>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='fotoalbum'),
    WgetDownload(
        WgetArgs(),
        max_tries=1,
